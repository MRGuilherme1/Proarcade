...
// Dados do jogo
let phase = "waiting"; // esperando | alongamento | torneamento | andando | transição | caindo
let lastTimestamp; // O carimbo de data/hora do ciclo requestAnimationFrame anterior

let heroX; // Mudanças ao seguir em frente
let heroY; // Mudanças ao avançarSó muda ao cair
...
// Todo: Salve a pontuação mais alta no localStorage(?)
...
const paddingX = 100; // A posição de espera do herói a partir do tamanho original da tela
...
canvas.width = window.innerWidth; // Deixe o Canvas em tela cheia
...
function resetGame() {
    ...
    // x + w deve corresponder a paddingX
    platforms = [{ x: 50, w: 50 }];
    generatePlatform();
    generatePlatform();
    generatePlatform();
...
// Retorna a plataforma em que o stick atingiu (se não atingiu nenhum stick, retorne indefinido)
function thePlatformTheStickHits() {
    ...
    // Se o stick atingir a área perfeita
    if (
...
function drawSticks() {
    sticks.forEach((stick) => {
        ...
        // Mova o ponto de ancoragem para o início do bastão e gire
